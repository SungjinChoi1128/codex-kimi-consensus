...
7. While the run is active, print a concise progress update when phase, heartbeat, artifact counts, Kimi status, live changed files, PRD/test-spec/OMX paths, or error changes. Example shape: `round 1/5 · codex.proposal · heartbeat 2 · waiting for Codex proposal`. During `codex.revision`, if `changed_files` is non-empty, show `Codex is editing: <paths>`.
8. At `AWAITING_HUMAN_APPROVAL`, show the generated `prd_path`, `test_spec_path`, `omx_plan_path`, and `context_bridge_path`. If `ralph_handoff_status` is `ready`, show `ralph_handoff_prompt` as the user-facing Codex CLI handoff. If it is `blocked`, show the blockers and guide instead. Do not replace a ready handoff with a terminal `consensusd approve` command. `approve_ralph_handoff` is audit bookkeeping for consensusd; the actual Ralph UX is pasting the `$ralph ...` prompt into Codex CLI.
...
Continuity note: real Codex/Kimi subprocesses are separate OS processes, but consensusd captures their CLI resume ids and resumes them on later rounds where supported. Treat the SQLite transcript and evidence packet as the durable source of truth, and the resumed CLI session as extra continuity.

Planning-bundle note: Codex round 1 should produce a draft PRD and draft test spec immediately. consensusd writes `draft-prd-r*-*.md` and `draft-test-spec-r*-*.md` before Kimi review, records `proposal_draft_quality`, and asks Kimi to review those concrete drafts. Do not treat a thin proposal as acceptable when the user asked for PRD/test-spec quality.
