...
Timeout warning: real Codex/Kimi phases default to 60 minutes. Codex proposal receives a bounded deep context packet first: objective commit diff, relevant current files, P11B/Revolut artifacts, package/test context, and capped repo-local file contents. It should understand that context properly, inspect only a few named extra files if needed, and name missing external API facts rather than inventing them.
